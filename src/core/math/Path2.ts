...
import { fromPoints, fromSides } from "../CAGFactories";
...
export class Path2
{
     ...
     * @returns {Path2} new Path2D object (not closed)
     */
    appendPoint(point)
    {
        if (this.closed)
    ...
    close()
    {
        return new Path2(this.points, true);
    }
    ...
    getTurn()
    {
        ...
        {
            twice_area +=
                points[last].x * points[current].y -
                points[last].y * points[current].x;
        }
        if (twice_area > 0)
        {
            return "clockwise";
    ...
    rectangularExtrude(width, height, resolution, round?)
    {
    ...
    transform(matrix4x4)
    {
        ...
        return new Path2(newpoints, this.closed);
    }
    ...
    appendBezier(controlpoints, options)
    {
        ...
        {
            throw new Error(
                "appendBezier: should pass an array of control points"
            );
        }
        ...
        for (let i = 0; i < controlpoints.length; ++i)
        {
            ...
            if (p === null)
            {
                ...
                else
                {
                    ...
                    lastBezierControlPoint = this.points[
        ...
        {
            let t_k = 1; // = pow(t,k)
            let one_minus_t_n_minus_k = (1 - t) ** bezierOrder; // = pow( 1-t, bezierOrder - k)
            let inv_1_minus_t = t !== 1 ? 1 / (1 - t) : 1;
            let point = new Vector2D(0, 0);
            for (let k = 0; k <= bezierOrder; ++k)
            {
                if (k === bezierOrder) one_minus_t_n_minus_k = 1;
                let bernstein_coefficient =
                    binomials[k] * t_k * one_minus_t_n_minus_k;
                point = point.plus(
                    controlpointsParsed[k].times(bernstein_coefficient)
                );
                t_k *= t;
        ...
        let result = new Path2(newpoints);
    ...
    appendArc(endpoint, options)
    {
        ...
        } else
        {
            ...
            let x =
                Math.round(
                    (cosphi * minushalfdistance.x +
                        sinphi * minushalfdistance.y) *
                    decimals
                ) / decimals;
            let y =
                Math.round(
                    (-sinphi * minushalfdistance.x +
                        cosphi * minushalfdistance.y) *
                    decimals
                ) / decimals;
            ...
            let numsteps =
                Math.ceil((Math.abs(deltatheta) / (2 * Math.PI)) * resolution) +
                1;
    ...
    static arc(options)
    {
        ...
        {
            point = Vector2D.fromAngle((startangle / 180.0) * Math.PI).times(
                radius
            );
        ...
        } else
        {
            ...
            for (let i = 0; i <= numstepsMod; i++)
            {
                ...
                let angle =
                    startangle + (step * (endangle - startangle)) / numsteps;
                point = Vector2D.fromAngle((angle / 180.0) * Math.PI).times(
                    radius
                );
                points.push(point.plus(center));
            }
        }
        return new Path2(points, false);
    }
}
