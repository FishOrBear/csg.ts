...
import { parseOption, parseOptionAs2DVector, parseOptionAs3DVector, parseOptionAs3DVectorList, parseOptionAsFloat, parseOptionAsInt } from "./optionParsers";
...
export function cube(options: {})
{
    let c: Vector3D;
    let r: Vector3D;
    let corner1: Vector3D;
    let corner2: Vector3D;
    options = options || {};
...
})
{
    ...
    let xvector: Vector3D, yvector: Vector3D, zvector: Vector3D;
    if ("axes" in options)
    {
    ...
    for (let slice1 = 0; slice1 <= resolution; slice1++)
    {
        ...
        if (slice1 > 0)
        {
            // cylinder vertices:
            let vertices = [];
            let prevcospitch: number, prevsinpitch: number;
    ...
    result.properties.sphere.center = Vector3D.Create(center);
 ...
 * });
 */
export function cylinder(options: any)
{
    ...
    let start = new Vertex3D(s);
    let end = new Vertex3D(e);
    let polygons = [];
    ...
    if (alpha > 0)
    {
        for (let i = 0; i < slices; i++)
        {
            ...
            {
                polygons.push(
                    new Polygon([start, point(0, t0, rEnd), point(0, t1, rEnd)])
                );
                polygons.push(
                    new Polygon([
                        point(0, t1, rEnd),
                ...
                polygons.push(
                    new Polygon([end, point(1, t1, rEnd), point(1, t0, rEnd)])
                );
            } else
            {
                if (rStart > 0)
        ...
        if (alpha < 360)
        {
            ...
            polygons.push(
                new Polygon([point(0, 0, rStart), end, point(1, 0, rEnd)])
            );
            polygons.push(new Polygon([start, point(0, 1, rStart), end]));
            polygons.push(
                new Polygon([point(0, 1, rStart), point(1, 1, rEnd), end])
            );
        }
    }
    let result = fromPolygons(polygons);
...
})
{
    ...
    let yvector = xvector
        ...
        .unit()
        .times(radius);
    let prevcylinderpoint: Vector3D;
    for (let slice1 = 0; slice1 <= resolution; slice1++)
    {
        let angle = (Math.PI * 2.0 * slice1) / resolution;
        let cylinderpoint = xvector
        ...
        if (slice1 > 0)
        {
            // cylinder vertices:
            let vertices = [];
            vertices.push(new Vertex3D(p1.plus(cylinderpoint)));
            vertices.push(new Vertex3D(p1.plus(prevcylinderpoint)));
            vertices.push(new Vertex3D(p2.plus(prevcylinderpoint)));
            vertices.push(new Vertex3D(p2.plus(cylinderpoint)));
            polygons.push(new Polygon(vertices));
            let prevcospitch: number, prevsinpitch: number;
            for (let slice2 = 0; slice2 <= qresolution; slice2++)
            {
...
export function cylinderElliptic(options: any)
{
    let s = parseOptionAs3DVector(options, "start", [0, -1, 0]);
    let e = parseOptionAs3DVector(options, "end", [0, 1, 0]);
...
})
{
    ...
    res.properties.roundedCube.center = new Vertex3D(center);
...
export function polyhedron(options: {})
{
    ...
    let vertices = parseOptionAs3DVectorList(options, "points", [
        ...
        [-1, 1, 0],
        [0, 0, 1]
    ]).map(function (pt: Vector3D)
    {
        return new Vertex3D(pt);
    });
    ...
    faces.forEach(function (face: { reverse: () => void })
    {
        face.reverse();
    });
    let polygons = faces.map(function (face: {
        map: (arg0: (idx: any) => any) => Vertex3D[];
    })
    {
        return new Polygon(
            face.map(function (idx: string | number)
            {
                return vertices[idx];
            })
        );
