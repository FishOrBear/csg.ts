...
export const contract = (shape, radius, resolution) =>
{
    let result;
    if (isCSG(shape))
    {
 ...
 */
export const expandedShellOfCCSG = (
    _csg: CSG,
    radius,
    resolution,
    unionWithThis?
    ...
    for (let vertextagpair in vertexpairs)
    {
        ...
        for (let i = -1; i < numangles; i++)
        {
            ...
            if (!skip)
            {
                if (i >= 0)
                {
                    startfacevertices.push(new Vertex3D(p1));
                    endfacevertices.push(new Vertex3D(p2));
                    let polygonvertices = [
                        new Vertex3D(prevp2),
                        new Vertex3D(p2),
                        new Vertex3D(p1),
