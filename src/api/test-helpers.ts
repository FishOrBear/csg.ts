export function vertex3Equals(t, observed, expected)
{
    const obs = [observed.pos._x, observed.pos._y, observed.pos._z];
    return t.deepEqual(obs, expected);
}

export function vertex2Equals(t, observed, expected?, debugStr?)
{
    const obs = [observed.pos._x, observed.pos._y];
    return t.deepEqual(obs, expected);
}

export function vector3Equals(t, observed, expected)
{
    const obs = [observed._x, observed._y, observed._z];
    return t.deepEqual(obs, expected);
}

export function sideEquals(t, observed, expected)
{
...
export function shape2dToNestedArray(shape2d)
{
...
export function shape3dToNestedArray(shape3d)
{
...
export function simplifiedPolygon(polygon)
{
    ...
    const plane = {
        normal: [
            polygon.plane.normal._x,
            polygon.plane.normal._y,
            polygon.plane.normal._z
        ],
        w: polygon.plane.w
    };
...
export function simplifiedCSG(csg)
{
    const polygonsData = csg.polygons.map(x => simplifiedPolygon(x).positions);
    return polygonsData;
}

export function almostEquals(t, observed, expected, precision)
{
    t.is(Math.abs(expected - observed) < precision, true);
}

export function compareNumbers(a, b, precision)
{
    return Math.abs(a - b) < precision;
}

export function compareVertices(a, b, precision)
{
    if ("_w" in a && !("_w" in b))
    {
        return false;
...
export function comparePolygons(a, b, precision)
{
